...
for station in stations:
    ...
    # Get the min, q1, median, q3, max std, and variance of each variable
    columns = data.columns[1:-2]
    for column in columns:
        minimm = data[column].min()
        if minimm < 0:
            minimm = 0
        results.loc[len(results.index)] = [column, minimm, data[column].quantile(0.25), data[column].median(), data[column].quantile(0.75), data[column].max(), data[column].std(), data[column].var()]
